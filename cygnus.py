...
while True:
    ...
    has <var>                            : Check if a variable exists
    math <var> = <expression>            : Evaluates an expression
    math <var> = <var> <expression>      : Creates a new variable and stores the data updated from one of the old variables  
    ''')
        else:
            format_error()
    ...
    # Math Command
        ...
        if len(parts) >= 4 and parts[2] == '=':
            var_name = parts[1]
            expression = ' '.join(parts[3:])  # Join the rest into an expression string
            ...
            if all(char in allowed for char in expression):
                try:
                    ...
                    print(Fore.GREEN + f">>> {var_name} = {result}")
                except Exception:
                    opr_error()  # In case eval fails
            else:
                print(Fore.RED + "!!! ~ Unsafe characters in expression")
        else:
            ass_error()

    
    # Adding Variables
    elif parts[0] == 'math' and parts[2] == '=':
        var_name = parts[1]
        expression = ' '.join(parts[3:])

        for name in variables:
            if name in expression:
                expression = expression.replace(name, str(variables[name]))

        if all(char in allowed_chars for char in expression):
            result = eval(expression)
            variables[var_name] = result
            print(">>>", var_name, "=", result)
        else:
